...
const PostStatusQueueClient_1 = require("./sqs/PostStatusQueueClient");
const FeedWriteQueueClient_1 = require("./sqs/FeedWriteQueueClient");
class StatusService extends Service_1.Service {
    ...
    async postStatus(token, newStatus) {
        ...
        const postStatusQueue = new PostStatusQueueClient_1.PostStatusQueueClient();
    ...
    async updateFeeds(message) {
        const feedWriteQueueUrl = process.env.FEED_WRITE_QUEUE_URL;
        if (!feedWriteQueueUrl) {
            throw new Error("internal-server-error: Missing env var FEED_WRITE_QUEUE_URL");
        }
        const { authorAlias, timestamp, post } = message;
        if (!authorAlias || !timestamp || !post) {
            throw new Error("bad-request: updateFeeds message missing required fields");
        }
        ...
        while (hasMore) {
             ...
             */
            const page = await this.follows.getFollowersPage(authorAlias, pageSize, lastFollowerAlias);
        ...
        const feedWriteQueue = new FeedWriteQueueClient_1.FeedWriteQueueClient();
        const sendPromises = [];
    ...
    async writeFeedBatch(message) {
        const { authorAlias, timestamp, post, followerAliases } = message;
        if (!authorAlias || !timestamp || !post) {
            throw new Error("bad-request: writeFeedBatch message missing status fields");
        }
        ...
        if (followerAliases.length > 25) {
            throw new Error("bad-request: writeFeedBatch received more than 25 followers");
        }
        /**
